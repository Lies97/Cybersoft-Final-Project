...
class LichPhim extends Component {
  ...
  handleHeThongRap = (item, index) => {
    ...
    x.map((item) => {
      ...
      const finalItem = item.filter(
        (nestedItem) => new Date(nestedItem.ngayChieuGioChieu).valueOf() > now
      );
      this.setState(
        {
          [rapTen]: finalItem,
  ...
  renderRapLogo = () => {
    ...
    return rap.map((item, index) => {
      return (
        <li key={index}>
          <div
            className={index === activeLink ? "logo active" : "logo"}
  ...
  renderHeThongRap = (data) => {
    if (data[0]) {
      ...
      return rap.map((item, index) => {
        ...
        return (
          <div
            className="row container-lich-phim"
            onClick={() => {
              this.handleHeThongRap(item, index);
            }}
          >
            ...
            <div className="col-9 cursor">
              <span className="colored-label">{coloredLabel}</span> -{" "}
              <span className="normal-label">{normalLabel}</span>
    ...
    return (
      ...
      >
        ...
        <div className="col-11 cursor">
          <span className="colored-label">text</span> -{" "}
          <span className="normal-label">text</span>
          <p className="grey-color">test</p>
        </div>
        <div className="col-12 text">2D Digital</div>
        <div className="col-12">
          <button className="time">data</button>
        </div>
      </div>
    );
  };
  ...
  render() {
    ...
    return (
      <div className="row LichPhim container-lich-phim" id="lichFilm">
        <div className="row container container-lich-phim">
          ...
          <div className="tab-content float-left col-4">
            ...
            {LotteCinima &&
              activeLink === 5 &&
              this.renderHeThongRap(LotteCinima)}
          </div>
          <div className="listMovies col-6">{this.renderListMovie()}</div>
        </div>
      </div>
    );
  }
